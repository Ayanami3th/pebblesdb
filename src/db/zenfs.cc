...
std::unique_ptr<ZonedBlockDevice> zbd_open(bool readonly, bool exclusive) {
  ...
  Status open_status = zbd->Open(readonly, exclusive);
...
Status zenfs_mount(std::unique_ptr<ZonedBlockDevice> &zbd,
  ...
  std::unique_ptr<ZenFS> localZenFS{
      new ZenFS(zbd.release(), Env::Default(), nullptr)};
...
int zenfs_tool_mkfs() {
  ...
  zenFS.reset(new ZenFS(zbd.release(), Env::Default(), nullptr));
...
void list_children(const std::unique_ptr<ZenFS> &zenFS,
  ...
  uint64_t size;
  Status io_status = zenFS->GetChildren(path, &result);
  ...
  for (const auto &f : result) {
    io_status = zenFS->GetFileSize(path + f, &size);
    if (!io_status.ok()) {
      fprintf(stderr, "Failed to get size of file %s\n", f.c_str());
      return;
    }
    uint64_t mtime;
    io_status = zenFS->GetFileModificationTime(path + f, &mtime);
    if (!io_status.ok()) {
      fprintf(stderr,
...
Status zenfs_tool_copy_file(Env *f_fs, const std::string &f,
                              Env *t_fs, const std::string &t) {
  // FileOptions fopts;
  IOOptions iopts;
  IODebugContext dbg;
  Status s;
  SequentialFile *f_file;
  WritableFile *t_file;
  ...
  s = f_fs->NewSequentialFile(f, &f_file);
  if (!s.ok()) {
    return s;
  }

  s = t_fs->NewWritableFile(t, &t_file);
  if (!s.ok()) {
    return s;
  }
  ...
  while (to_copy > 0) {
    ...
    s = f_file->Read(chunk_sz, &chunk_slice, buffer.get());
    if (!s.ok()) {
      break;
    }
...
Status zenfs_tool_copy_dir(Env *f_fs, const std::string &f_dir,
                             Env *t_fs, const std::string &t_dir) {
  IOOptions opts;
  IODebugContext dbg;
  Status s;
...
Status zenfs_create_directories(Env *fs, std::string path) {
  std::string dir_name;
  IODebugContext dbg;
  IOOptions opts;
  Status s;
  ...
  while ((p = path.find_first_of('/', p)) != std::string::npos) {
    ...
    s = fs->CreateDirIfMissing(dir_name);
    if (!s.ok()) break;
  }
...
int zenfs_tool_backup() {
  ...
  bool is_dir;
  io_status = zenFS->IsDirectory(FLAGS_backup_path, &is_dir);
  if (!io_status.ok()) {
  ...
  if (!is_dir) {
    ...
    io_status =
        zenfs_tool_copy_file(zenFS.get(), FLAGS_backup_path,
                             Env::Default(), dest_filename);
  } else {
    io_status =
        zenfs_create_directories(Env::Default(), FLAGS_path);
    if (!io_status.ok()) {
...
int zenfs_tool_link() {
  ...
  io_s = zenFS->LinkFile(FLAGS_src_file, FLAGS_dest_file);
  if (!io_s.ok()) {
...
int zenfs_tool_delete_file() {
  ...
  io_s = zenFS->DeleteFile(FLAGS_path);
  if (!io_s.ok()) {
...
int zenfs_tool_rename_file() {
  ...
  io_s = zenFS->RenameFile(FLAGS_src_file, FLAGS_dest_file);
  if (!io_s.ok()) {
...
int zenfs_tool_remove_directory() {
  ...
  if (FLAGS_force) {
    io_s = zenFS->DeleteDirRecursive(FLAGS_path);
    if (!io_s.ok()) {
      fprintf(stderr,
    ...
    io_s = zenFS->DeleteDir(FLAGS_path);
    if (!io_s.ok()) {
...
int zenfs_tool_restore() {
  ...
  Env *f_fs = Env::Default();
  status = f_fs->IsDirectory(FLAGS_path, &is_dir);
  if (!status.ok()) {
    fprintf(stderr, "IsDirectory failed, error: %s\n",
            status.ToString().c_str());
    return 1;
...
int main(int argc, char **argv) {
  ...
  if (subcmd == "mkfs") {
    return leveldb::zenfs_tool_mkfs();
  } else if (subcmd == "list") {
    return leveldb::zenfs_tool_list();
  } else if (subcmd == "ls-uuid") {
    return leveldb::zenfs_tool_lsuuid();
  } else if (subcmd == "df") {
    return leveldb::zenfs_tool_df();
  } else if (subcmd == "backup") {
    return leveldb::zenfs_tool_backup();
  } else if (subcmd == "restore") {
    return leveldb::zenfs_tool_restore();
  } else if (subcmd == "dump") {
    return leveldb::zenfs_tool_dump();
  } else if (subcmd == "fs-info") {
    return leveldb::zenfs_tool_fsinfo();
  } else if (subcmd == "link") {
    return leveldb::zenfs_tool_link();
  } else if (subcmd == "delete") {
    return leveldb::zenfs_tool_delete_file();
  } else if (subcmd == "rename") {
    return leveldb::zenfs_tool_rename_file();
  } else if (subcmd == "rmdir") {
    return leveldb::zenfs_tool_remove_directory();
  } else {
