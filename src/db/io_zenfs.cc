...
void ZoneFile::SetIOType(Env::IOType io_type) { io_type_ = io_type; }
...
Status ZoneFile::AllocateNewZone() {
  ...
  if (!s.ok()) return s;
  if (!zone) {
    return Status::IOError("Zone allocation failure\n");
  }
  SetActiveZone(zone);
  extent_start_ = active_zone_->wp_;
  extent_filepos_ = file_size_;
...
Status ZoneFile::SparseAppend(char* sparse_buffer, uint32_t data_size) {
  uint32_t left = data_size;
  uint32_t wr_size;
  uint32_t block_sz = GetBlockSize();
  Status s;
...
Status ZoneFile::RecoverSparseExtents(uint64_t start, uint64_t end,
                                        Zone* zone) {
  ...
  if (ret) {
    return Status::IOError("Out of memory while recovering");
  }
  ...
  while (next_extent_start < end) {
    ...
    if (ret != (int)block_sz) {
      s = Status::IOError("Unexpected read error while recovering");
      break;
    }

    extent_length = DecodeFixed64(buffer);
    if (extent_length == 0) {
      s = Status::IOError("Unexexpeted extent length while recovering");
      break;
    }
    recovered_segments++;
...
Status ZoneFile::Recover() {
  ...
  if (is_sparse_) {
    Status s = RecoverSparseExtents(extent_start_, zone->wp_, zone);
    if (!s.ok()) return s;
  } else {
...
Status ZonedWritableFile::Fsync() {
  ...
  ZenFSMetricsLatencyGuard guard(zoneFile_->GetZBDMetrics(),
                                 zoneFile_->GetIOType() == Env::IOType::kWAL
  ...
  if (buffered && !zoneFile_->IsSparse()) return Status::OK();
...
Status ZoneFile::MigrateData(uint64_t offset, uint32_t length,
                               Zone* target_zone) {
  ...
  if (offset % block_sz != 0) {
    return Status::IOError("MigrateData offset is not aligned!\n");
  }
